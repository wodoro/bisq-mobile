 ...
 *
 * Reporting [isSupported] as false hides the settings opt-in (see
 * `SettingsPresenter.shouldShowPushNotificationsToggle`), so these methods exist to keep the
 * contract total rather than to be called. They still fail rather than fake success: a silent
 * `Result.success` would register the device with the trusted node under a token nothing can
 * deliver to, and the user would believe push is working.
 */
